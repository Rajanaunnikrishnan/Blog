...
def cart_details(request, tot=0, cart_items=None, c_id=None, count=None):
    try:
        ct=cartlist.objects.get(cart_id=c_id(request))
        ct_items=items.objects.filter(ct=ct,active=True)
        for i in ct_items:
...
def add_cart(request,product_id,c_id):
